package com.kt.smp.fileutil.builder;
...
/**
 * The type Sxssf excel builder.
 *
 * @author jaime
 * @title SxssfExcelBuilder
 * @see\n <pre> </pre>
 * @since 2022 -03-07
 */
@Slf4j
public class SxssfExcelBuilder {
    ...
    public static SXSSFWorkbook createExcel(List<String> headers
         ...
         */
        String checkEmptySheetName = StringUtils.isEmpty(sheetName)
                ? SHEET_NAME + (rowIdx / MAX_ROW + 1) : sheetName;
        boolean newSheet = ObjectUtils.isEmpty(workbook.getSheet(checkEmptySheetName));
        Sheet sheet = newSheet ? workbook.createSheet(checkEmptySheetName) : workbook.getSheet(checkEmptySheetName);
