    ...
    function () {
      ...
      _createClass(MaterialDatepicker, [{
        ...
        value: function _define() {
          ...
          this._createElement();
          this.eventListener = {};    //  나중에 removeEventListener 하기 위해서 저장
          ...
          this.element.addEventListener(this.settings.openOn, this.eventListener[this.settings.openOn + ".1"] = function () {
            _this.open(_this.settings.openOn);
          });

          if (this.settings.openOn != 'direct') {

            document.addEventListener('keyup', this.eventListener['keyup.1'] = function (e) {
              if (e.keyCode == 9) {
                // TAB watch
            ...
            document.addEventListener('keyup', this.eventListener['keyup.2'] = function (e) {
            ...
            document.addEventListener('mouseup', this.eventListener['mouseup'] = function (event) {
              var isPicker = false;
        ...
        value: function _createElement(time) {
          ...
          // 23.12.07 스타일 태그 대신 page.scss에 해당 스타일 추가
          this.draw();
          this.callbackOnLoad();
        }
      }, {
        ...
        value: function destroy() {
          // 2022.11.30 - 다른 화면으로 이동해도 이벤트가 남아 있어서 binding 해지 함수 별도로 생성
          this.element.removeEventListener(this.settings.openOn, this.eventListener[this.settings.openOn + ".1"]);
          document.removeEventListener('keyup', this.eventListener['keyup.1']);
          document.removeEventListener('keyup', this.eventListener['keyup.2']);
          document.removeEventListener('mouseup', this.eventListener['mouseup']);
        }
      }]);
